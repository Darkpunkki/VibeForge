...
export function registerResolveIdeaId(server: McpServer) {
  server.registerTool(
    'vf.resolve_idea_id',
    {
      title: 'Resolve IDEA id to idea folder',
      description:
        'Resolves an IDEA reference (IDEA-0001 or IDEA-0001-slug) to the actual idea folder name under docs/forge/ideas/.',
      inputSchema: {
    ...
    async ({ idea_ref, repo_root }: { idea_ref: string; repo_root?: string }) => {
      ...
      if (!isShortId(ref)) {
        throw new Error(
          `Cannot resolve "${ref}". Provide IDEA-XXXX or full folder name like IDEA-XXXX-some-slug.`
        );
      }
      ...
      if (matches.length > 1) {
        throw new Error(
          `Multiple idea folders match "${ref}-":\n- ${matches.join('\n- ')}\nUse the full folder name.`
        );
      }
